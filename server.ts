...
const CHECKING_APPS_SCRIPT_URL = process.env.CHECKING_APPS_SCRIPT_URL || MARATHON_APPS_SCRIPT_URL;
...
async function startServer() {
  ...
  function normalizeRaceObject(raw: any): any {
    ...
    return {
      ...
      checkingScriptUrl: (raw.checkingScriptUrl || '').trim(),
  ...
  app.post('/api/admin/races', (req, res) => {
    try {
      ...
      const newRaceObj = {
        ...
        checkingScriptUrl: (race.checkingScriptUrl || '').trim(),
  ...
  // Dedicated Logging API for Certificate HD Downloads (Sheet: CHECKING)
  app.post('/api/log-download', async (req, res) => {
    try {
      const { timestamp, bib, race, time, scriptUrl } = req.body;
      const targetScriptUrl =
        scriptUrl ||
        process.env.CHECKING_APPS_SCRIPT_URL ||
        CHECKING_APPS_SCRIPT_URL ||
        MARATHON_APPS_SCRIPT_URL;

      if (!targetScriptUrl) {
        return res.status(400).json({ error: 'Chưa cấu hình Google Apps Script URL cho sheet CHECKING' });
      }
      ...
      console.log(`[Log Download] Logging to Apps Script: BIB=${bib}, RACE=${race}, TIME=${time}`);
      ...
      console.warn('[Log Download] Error sending log to Apps Script:', err.message);
      return res.status(500).json({ error: err.message || 'Lỗi khi ghi log tải ảnh' });
    }
  });

  app.get('/api/log-download', async (req, res) => {
    try {
      const { timestamp, bib, race, time, scriptUrl } = req.query;
      const targetScriptUrl =
        (scriptUrl as string) ||
        process.env.CHECKING_APPS_SCRIPT_URL ||
        CHECKING_APPS_SCRIPT_URL ||
      ...
      const urlObj = new URL(targetScriptUrl);
      urlObj.searchParams.set('action', 'checking');
      if (timestamp) urlObj.searchParams.set('TIMESTAMP', String(timestamp));
      if (bib) urlObj.searchParams.set('BIB', String(bib));
      if (race) urlObj.searchParams.set('RACE', String(race));
      if (time) urlObj.searchParams.set('TIME', String(time));
